...
$(document).ready(function () {
  ...
  $(".rating-modal-btn").on("click", function () {
    const bookingId = $(
      '#rating-modal .modal-footer input[name="bookingId"]'
    ).val();
  ...
  //   minDate: "today",
  //   dateFormat: "d M Y",
  //   plugins: [new rangePlugin({ input: "#dateFrom" })],
  // });
  ...
  function renderSearchResultFilter(result) {
    ...
    if (result.lastPage < 1 || result.index > result.lastPage) {
      ...
      for (let i = 1; i <= result.lastPage; i++) {
        pagination +=
          '                                    <li class="page-item ' +
          (i == result.index ? "active" : "") +
          ' ">\n' +
          '                                        <input type="hidden" name="index" value="' +
          i +
          '">\n' +
          '                                        <div class="page-link ">0' +
          i +
          ".</div>\n" +
          "                                    </li>";
      }
  ...
  function renderModal(result) {
    var context = window.location.pathname.substring(
      0,
    ...
    for (let i = 0; i < image.length; i++) {
      if (image[i] !== "") {
        img +=
          ...
          i +
          '" class="col-2 p-0 mx-1 ';
        if (i == 0) {
          img += "active";
        }
        img +=
          '">\n' +
          '                                            <img class="d-block w-100 p-1" src="' +
          (context + "/uploads/room/" + image[i]) +
          '">\n' +
          "                                        </li>";
      }
    ...
    let content =
      ...
      "                                </div>\n" +
      '                                <div class="mb-3">\n' +
      '                                    <p class="text-dark m-0" style="font-size: 15px;"><strong>Kích thước phòng</strong> ' +
      room.area +
      " m<sup>2</sup></p>\n" +
      "                                </div>\n" +
      '                                <div class="mb-3">\n' +
    ...
    if (room.dinningroom !== null && room.dinningroom !== "") {
      ...
      for (let i = 0; i < dinningroom.length; i++) {
        content +=
          '<li><i class="fa-solid fa-check mr-2 text-success"></i>' +
          dinningroom[i] +
          "\n<li>\n";
      }
      content += "</ul>\n" + "                                    </div>\n";
    ...
    const lastPrice =
      (parseFloat(room.price) *
        subDate *
        (100 - parseInt(room.discountPercent))) /
      100;
    let lastPriceRight =
      "<div>\n" +
      '                                    <span class="text-danger mr-2"\n' +
      '                                          style="text-decoration: line-through;">VND ' +
      Math.trunc(room.price * subDate).toLocaleString("pl-PL") +
      " </span><span\n" +
      '                                        class="bg-danger text-white rounded" style="padding: 2px 5px;">-' +
      room.discountPercent.toLocaleString("pl-PL") +
      "%</span>\n" +
      "                                </div>\n" +
      '                                <strong class="text-success mb-2" style="font-size: 20px;">VND ' +
      Math.trunc(lastPrice).toLocaleString("pl-PL") +
      " cho " +
      subDate +
      "\n" +
      "                                    đêm</strong>\n" +
